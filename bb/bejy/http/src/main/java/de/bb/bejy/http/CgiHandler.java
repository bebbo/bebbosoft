...
public class CgiHandler extends HttpHandler {
	...
	private final static String PROPERTIES[][] = { { "command", "the CGI command line" },
			...
			{ "fastCgiHost", "name of the FastCGI host" } };
	...
	private FastCGIHost fastCgiHost;
	...
	public void activate(LogFile logFile) throws Exception {
		...
		String fastCgiHostName = getProperty("fastCgiHost");
		if (fastCgiHostName != null) {
			fastCgiHost = Config.getConfigsByName(FastCGIHost.class).get(fastCgiHostName);
		}
	}
	...
	public void service(javax.servlet.ServletRequest request, javax.servlet.ServletResponse response)
		...
		int inContentLength = hreq.getContentLength();
		if (inContentLength < 0) {
			inContentLength = is.available();
		}
		...
		if (inContentLength > 0)
			v.add("CONTENT_LENGTH=" + inContentLength);
		...
		if (fastCgiHost != null) {
			fastCgiHandle(v, is, os, inContentLength, sr);
			return;
		}
		...
		if (DEBUG) {
			byte b[];
			if (inContentLength >= 0) {
				b = new byte[inContentLength]; // b[cl-2] = 0xd; // b[cl-1] =
												// 0xa;

				if (inContentLength > 0) {
					for (int pos = 0; pos < inContentLength;) {
						int len = is.read(b, pos, inContentLength - pos);
						if (len == 0)
							break;
						pos += len;
					}
	...
	private void fastCgiHandle(ArrayList<String> env, InputStream is, OutputStream os, int inContentLength,
			HttpResponse sr) throws IOException {
		InputStream fcgiis = null;
		OutputStream fcgios = null;
		...
		try {
			for (int i = 0; i <= fastCgiHost.getMaxCount(); ++i) {
				try {
					...
					FastCGIHost.addHeader(fcgios, 1, FastCGIHost.BEGIN_REQUEST_BODY);
					FastByteArrayOutputStream params = new FastByteArrayOutputStream();
					for (final String s : env) {
						int eq = s.indexOf('=');
						FastCGIHost.addParam(params, s.substring(0, eq), s.substring(eq + 1));
					}
					FastCGIHost.addHeader(fcgios, 4, params.toByteArray());
					FastCGIHost.addHeader(fcgios, 4, FastCGIHost.NULL_BYTES);
					// fcgios.flush();
					...
					while (inContentLength > 0) {
						...
						buffer[0] = (byte) b0;
						int toRead = (inContentLength < buffer.length ? inContentLength : buffer.length) - 1;
						int read = 1;
						if (toRead > 0)
							read += is.read(buffer, 1, toRead);
						FastCGIHost.addFcgi(fcgios, 5, buffer, read);
						inContentLength -= read;
					}

					FastCGIHost.addHeader(fcgios, 5, FastCGIHost.NULL_BYTES);
					fcgios.flush();
			...
			if (fcgiis == null || fcgios == null)
				throw new IOException("no valid FastCGIHost");
