 ...
 */
class LongHash<T> {
    private final static int primes[] = { 53, 97, 193, 389, 769, 1543, 3079, 6151, 12289, 24593, 49157, 98317, 196613,
    ...
    public synchronized T get(long key) {
        ...
        final int i = ((int) key & 0x7FFFFFFF) % t.length;
        for (Entry<T> e = t[i]; e != null; e = e.next) {
            if (e.key == key) {
                return e.val;
            }
        }
     ...
     * entries more efficiently. This method is called automatically when the number of keys in the hashtable exceeds
    ...
    private void rehash() {
        ...
        threshold += threshold;
        final int newCapacity = primes[++primeIndex];

        @SuppressWarnings("unchecked")
        final Entry<T> t[] = new Entry[newCapacity];
        table = t;

        for (int i = oldCapacity; i-- > 0;) {
            for (Entry<T> f, e = oldTable[i]; e != null; e = f) {
                f = e.next;
                final int j = ((int) e.key & 0x7FFFFFFF) % newCapacity;
                e.next = t[j];
                t[j] = e;
            }
        }
    ...
    public synchronized Object remove(long key) {
        ...
        final int i = ((int) key & 0x7FFFFFFF) % t.length;
        // search key
        for (Entry<T> f = null, e = t[i]; e != null; e = e.next) {
            if (e.key == key) {
                --count;
                if (f == null) {
                    t[i] = e.next;
