...
<html>
  ...
  <body style="background-color:#FeeBa1;margin:0px;" onLoad='javascript:reframe()'>
    <%
      ...
      {
        %>
        <hr><b><font color="#ff0000"><%=e.getMessage()%></font></b><hr>
        Please check:
    ...
    if (po != null && po.length() > 0)
    {
	  ...
	  long poBoxSize = mDbi.getPoBoxSize(po, domain);
    ...
    <form action="mail.jsp" target="_self" method="POST">
      ...
      <table border='1'>
        <tr>
          ...
          <td><%=poBoxSize%> / </td>
          <td valign="top"><input alt="NOTE: set this value to 0 to disable mail box quota!"  type="text" name="quota" value="<%=quota%>" maxlength="16"></td>
        </tr>
        <tr>
          <td>keep received mail<br></td>
          <td valign="top" colspan="2"><input alt="NOTE: if you uncheck this option, the mail account acts as a NOREPLY account.All incoming mail is discarded! There is no way do recover those lost mails!"  type="checkbox" name="keep" <%=checked%> ></td>
        </tr>
        <tr>
          <td>auto reply</td>
      ...
      <table border='1'>
        ...
        <tr><td><input type="text" name="forwarder" value=""></td>
    ...
    if (action.equals("create"))
    {
      ...
      boolean sup = "/\\".equals(request.getParameter("$up"));
    ...
    <% } %>
    <table border='1'>
      <tr><td>Filter:</td><td colspan="5" >
        <form action="mail.jsp" target="_self" method="POST">
          <input type="text" name="$filter" value="<%=filter%>"/>
          <input type="submit" name="$up" value="<%=sup ? "\\/" : "/\\" %>" />
        </form></td></tr>          
      <%
        Vector domains = mDbi.getMailDomainsLike(owner, filter, sup);
      ...
      <tr><th align="left" colspan="6">&nbsp;</th></tr>
      <tr></tr>
    </table>
    <% if (result != null && result.length() > 0) { %>
      <b><font color='#ff0000'><%=result%></font></b>
