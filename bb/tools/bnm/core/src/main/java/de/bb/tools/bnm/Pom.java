...
public class Pom {
    ...
    private void expandLifeCycle(String lastPhase, ArrayList<String> commands)
            throws Exception {
        ...
        MultiMap<String, String> m = (MultiMap<String, String>) LifecycleManager
                .getPhases(builds, effectivePom.packaging, lifecycleId);
        if (m == null)
            return;
        for (String currentPhase : LifecycleManager
                .getGlobalPhases(lifecycleId)) {
            if (startPhase != null && !currentPhase.equals(startPhase))
                continue;
            startPhase = null;
            commands.addAll(m.subMap(currentPhase, currentPhase + "\0")
                    .values());
    ...
    /**
     * Creates the effective pom by using the parents effective pom and merging
     * it with our own pom. Merging with own pom overrides existing settings
     * 
     * @throws Exception
     */
    private void createEffectivePom() throws Exception {
        ...
        if (parent != null && parent.pom.version == null
                && pom.parent.version != null) {
            String old = parent.getId();
            parent.pom.version = pom.parent.version;
            Log.getLog().warn(
                    "patching parent version from:" + old + " to "
                            + parent.getId());
        }
        if (parent != null && parent.pom.version != null && pom.version == null) {
            ...
            Log.getLog().warn(
                    "patching version from parent:" + old + " to "
                            + parent.getId());
        }
        if (parent != null && pom.parent != null
                && !parent.pom.getId().equals(pom.parent.getId())
        ...
        this.loader = new Loader(effectivePom.repositories, null,
                loader != null ? loader : bnm.rootLoader);
        if (updateRootLoader)
        ...
        if (effectivePom.build != null) {
            for (Plugin p : effectivePom.build.plugins) {
                ...
                if (p.groupId == null) {
                    Log.getLog()
                            .warn("[" + pom.getId()
                                    + "] setting groupId of plugin "
                                    + p.getId() + " = org.apache.maven.plugins");
                ...
                if (!"false".equals(p.inherited)) {
                    /*
                     * // merge plugin configuration with plugin managament
                     * configuration MultiMap c = (MultiMap)
                     * pluginManagement.get(ga); if (c != null) {
                ...
                if (p.extensions) {
                    ...
                    InputStream is = loader.findInputStream(pid, "jar",
                            "META-INF/plexus/components.xml");
                    ArrayList<Component> cl = LifecycleManager
                            .getComponents(is);
                ...
                for (Execution pe : p.executions) {
                    ...
                    Execution ppe = (Execution) pluginManagement.get(ga + ":"
                            + pe.id);
                    if (ppe != null) {
                        Bind.merge(pe, ppe);
                    }
    ...
    private void applyPluginManagement() {
        ...
        if (effectivePom.build != null
                && effectivePom.build.pluginManagement != null) {
            for (Plugin p : effectivePom.build.pluginManagement.plugins) {
                ...
                String ga = p.getGA();
                if (p.replaces != null) {
                    for (final String r : p.replaces) {
                        replacementMap.put(r, ga);
                    }
                }
                /*
                 * pluginManagement.put(ga, p.configuration); for
                 * (PluginExecution pe : p.executions) { pluginManagement.put(ga
                 * + ":" + pe.id, pe); }
                 */
    ...
    private HashMap<String, String> resolveVars(LinkedList<Project> pomsInOrder)
            throws Exception {
        ...
        while (unresolvedVars.size() > 0) {
            ...
            for (Iterator<Map.Entry<String, String>> i = unresolvedVars
    ...
    private HashMap<String, String> getBuildVars() {
        ...
        String absPath = currentDir == null ? "." : currentDir
                .getAbsolutePath();
        buildVars.put("basedir", absPath);
        buildVars.put("project.directory", absPath);
        buildVars.put("project.build.directory", absPath + "/target");
        buildVars.put("project.build.outputDirectory", absPath
                + "/target/classes");
        buildVars.put("project.build.testOutputDirectory", absPath
                + "/target/test-classes");
        buildVars.put("project.build.sourceEncoding", "utf-8");
        return buildVars;
    }

    private void updateVars(HashMap<String, String> unresolvedVars,
            HashMap<String, String> resolvedVars) {
    ...
    private AbstractPlugin configure(Id id, ClassLoader cl, Mojo mojo,
            Map<String, String> conf, ArrayList<Dependency> dependencies)
            throws Exception {
        try {
            AbstractPlugin o = (AbstractPlugin) cl.loadClass(
                    mojo.implementation).newInstance();
    ...
    public void process(String phaseOrPlugin) throws Exception {
        ...
        File depFile = loader.findFile(effectivePom, null,
                effectivePom.packaging, false);
        ...
        if (!modified) {
            for (Id dep : getAllDependencies()) {
                ...
                depFile = loader.findFile(dep, ((Dependency) dep).classifier,
                        "jar", false);
                if (depFile == null || !depFile.exists()) {
                    modified = true;
                    break;
     ...
     * 
     * @param ga
     *            the GA of the plugin
     * @param goal
     *            the goal
     * @param dconf
     *            the defined configuration, if any
     * @param dependencies
     * @throws Exception
     */
    private void execute(String ga, String goal) throws Exception {
        ...
        try {
            instance = configure(id, pi.getClassLoader(), mojo, conf,
                    dependencies);
            ...
            String msg = "";
            try {
                int result = Process.execute(cmd, null, System.out, null,
                        0x1000000000000000L, this.currentDir);
                if (result == 0)
                    return;
            ...
            Log.getLog().error(
                    "no BNM plugin replacement for " + ga
                            + " and invoking mvn failed for " + ga + msg);
            return;
        } catch (Throwable e) {
        ...
        if (pi == null) {
            ...
            synchronized (pluginInfos) {
                ...
                if (pi == null) {
                    ...
                    ArrayList<Id> classpath = pluginPom
    ...
    public final ArrayList<Object> getDependencyTree(String sscope)
            throws Exception {
    ...
    private ArrayList<Object> getDependencyTree(HashSet<String> done,
        ...
        for (Dependency dep : effectivePom.dependencies) {
            ...
            if (dep.version == null) {
                Log.getLog().warn(
                        "no version for " + ga + " using own version "
                                + effectivePom.version);
            ...
            if (scope.ordinal() < DEPS[depScope].length) {
                ...
                if (sfollowScope != null) {
                    Scope followScope = Scope.valueOf(sfollowScope
                            .toUpperCase());
                    if (followScope == Scope.COMPILE
                            || followScope == Scope.TEST) {
                        pending.add(dep);
                    }
                }
            }
        ...
        for (Dependency dep : pending) {
            try {
                ...
                ArrayList<Object> list = transPom.getDependencyTree(done,
                        depScope);
                result.add(list);
            } catch (Exception e) {
    ...
    public ArrayList<Id> getRuntimeDependencies() throws Exception {
        ...
        for (Dependency d : effectivePom.dependencies) {
             ...
             * ("org.codehaus.plexus:plexus-container-default".equals(ga))
             * continue; if
            ...
            if (hit.contains(ga))
                continue;
            if (d.version == null) {
                throw new Exception("no version for: [" + pom.getId() + "] -> "
                        + d.getId());
        ...
        while (stack.size() > 0) {
            ...
            for (Dependency d : transPom.effectivePom.dependencies) {
                 ...
                 * ("org.apache.maven:maven-artifact-manager".equals(ga))
                 * continue; if
                 * ("org.apache.maven.wagon:wagon-provider-api".equals(ga))
                 * continue; if
                 * ("org.apache.maven.surefire:surefire-booter".equals(ga))
                 * continue; if
                 * ("org.codehaus.plexus:plexus-container-default".equals(ga))
                 * continue; if
                 * ("org.codehaus.plexus:plexus-classworlds".equals(ga))
                 * continue;
                 */
                /*
                 * if ("commons-logging:commons-logging".equals(ga)) { d.version
                 * = "1.0.4"; } else if ("log4j:log4j".equals(ga)) { d.version =
                 * "1.2.13"; }
                 */
                if ("compile".equals(d.scope)) {
                    if (hit.contains(ga))
                    ...
                    stack.add(d);
                    if (d.version == null) {
                        throw new Exception("no version for ["
                                + transDep.getId() + "] -> " + d.getId());
                    }
                    if (d.groupId == null) {
                        throw new Exception("no group id for ["
                                + transDep.getId() + "] -> " + d.getId());
        ...
        while (stack.size() > 0) {
            ...
            for (Dependency d : transPom.effectivePom.dependencies) {
                ...
                if ("compile".equals(d.scope)) {
                    ...
                    if (d.groupId == null) {
                        Log.getLog().warn(
                                "no group id for [" + transDep.getId()
                                        + "] -> " + d.getId() + " using "
                                        + transDep.groupId);
