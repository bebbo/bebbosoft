...
public class Loader {
    ...
    public Loader(ArrayList<Repository> repositories, ArrayList<Repository> pluginRepositories, Loader parentLoader) {
     ...
     * @param inside
     * @param mustExist
     *            force the existence of that file.
     * @return
     * @throws Exception
     */
    public URL findURL(Id id, String classifier, String ext, String inside, boolean mustExist) throws Exception {
        if (id.version == null)
        ...
        if (mustExist) {
            synchronized (localPoms) {
                if (!repoFile.exists()
                        || (downloadSnapshots && id.version != null && id.version.endsWith("SNAPSHOT") && !localPoms
                                .contains(id.getGA()))) {
    ...
    class Group {
        ...
        public synchronized void add(File file, File repoFile, Loader loader, ArrayList<Repository> repos,
                ArrayList<Repository> pomPluginRepositories) {
    ...
    public PluginParams getComponent(Plugin p) throws Exception {
        PluginParams pp = (PluginParams) pluginParams.get(p.getId());
        if (pp != null)
            return pp;
    ...
    public File findFile(Id id, String classifier, String ext, boolean mustExist) throws Exception {
