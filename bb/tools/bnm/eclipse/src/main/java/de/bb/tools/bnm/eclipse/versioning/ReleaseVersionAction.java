...
public class ReleaseVersionAction extends PomAction {

    public ReleaseVersionAction() {
        super(true, true);
    }
    ...
    public void run(IAction action) {
        ...
        try {
            ReleaseVersionRefactoring r = new ReleaseVersionRefactoring(currentProject);

            ReleaseVersionRefactoringWizard w = new ReleaseVersionRefactoringWizard(r, 2);

            RefactoringWizardOpenOperation op = new RefactoringWizardOpenOperation(w);
            op.run(shell, w.getWindowTitle());
        } catch (Exception e) {
