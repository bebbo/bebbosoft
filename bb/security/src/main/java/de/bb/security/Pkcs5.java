...
public class Pkcs5 {
    ...
    public static byte[] pb2decrypt(byte in[], String passwd) {
        try {
            ...
            byte iv[] = Asn1.getSeq(seq, pad, 0); // get used initialization vector
    ...
    public static boolean verifyPBKDF2(final String encodedPassword, final String password) {
        try {
            ...
            // must start with {....}
            final int ce = encodedPassword.indexOf('}');
            if (ce <= 0)
                return false;
            ...
            final String algo = palgo.substring(2);
     ...
     */
    public static String encodePbkdf2(final String algo, final String password, int n2Passes) {
        try {
            final MessageDigest md = MessageDigest.get(algo);
            if (md == null)
            ...
            byte[] encoded = Mime.encode(bos.toByteArray());

            final String encodedPassword = "{P5" + algo + "}" + new String(encoded, 0);
...
/*
 * $Log: Pkcs5.java,v $
 * Revision 1.5  2014/06/23 15:51:15  bebbo
