...
public class Asn1 implements Iterator<Asn1> {
	...
	public ByteRef getContent() {
		int len = getLen(data, offset);
		int header = getHeaderLen(data, offset);
		return new ByteRef(data, offset + header, offset + len);
	}
	...
	public static byte[] makeASN1(String s, int typ) {
		String cs = typ == UTF8String ? "utf-8" : typ == UTF16String ? "utf-16be" : "ISO-8859-1";
		try {
	 ...
	 * elements When 0x80 is or'd to the type, it means, that the given element is
	 * entered some examples path = 0x10: the first sequence is searched and return
	 * with header it IS the complete sequence path = 0x90: the first sequence is
	 * searched and its content is returned it IS the complete content of the
	 * sequence WITHOUT header path = 0x10, 0x90, 0x84: 0x10, search first sequence,
	 * do not enter 0x90, search next sequence, ENTER that sequence 0x84, search bit
	 * string, return its content!
	 * 
	 * @param b
	 ...
	 * @param off
	 *            an offset into b.
	 * @return a new allocated byte array which represent the searched element, or
	 *         null when not found.
	 */
	// ===========================================================================
	static public byte[] getSeq(byte b[], int s[], int off) {
		if (b == null)
			return null;
		if (s == null)
			return b;
	 ...
	 */
	static public int getInt(byte b[], int s[]) {
		b = getSeq(b, s, 0);
		if (b == null)
			return 0;
		int z = 0;
	 ...
	 */
	// ===========================================================================
	static public long getLong(byte b[], int s[]) {
		b = getSeq(b, s, 0);
		if (b == null)
			return 0;
		long z = 0;
	 ...
	 * @return the offset into the byte array where the searched element was found,
	 *         or -1.
	 */
	public static int searchSequence(byte b[], byte what[]) {
		int off[] = { 0 };
	 ...
	 *         or -1.
	 */
	public static int searchSequence(byte b[], int off, byte what[]) {
		int of[] = { off };
	 ...
	 *         or -1.
	 */
	public static int searchSequence(byte b[], int off[], byte what[], int end) {
	 ...
	 */
	static public boolean equals(byte a[], int ai, byte b[], int bi, int len) {
		for (int i = 0; i < len; ++i)
			if (a[i + ai] != b[i + bi])
				return false;
		return true;
