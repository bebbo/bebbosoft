...
/**
 * Base class for Ssl3Client and Ssl3Server. Contains all common functions, which are needed to establish an SSL3
 * connection.
 */
public abstract class Ssl3 {
    ...
    /**
     * Creates a new Ssl3 object, which uses the given InputStream for reading and the given OutputStream for writing.
     * 
    ...
    /**
     * Reads into the given byte array from input stream with blocking. This functions returns with an partial read too!
    ...
    /**
     * Writes the given byte array to the output stream. If necessary the block is split into smaller packets.
     * 
     * @param b
    ...
    /**
     * Reads the next SSL3 block into internal buffer. This function blocks until the complete block is read. An hack
     * allows to read also an SSL2 hello message header!
     * 
     * @return
     * @exception java.io.IOException
     *                throws an IOException if an I/O Error occurs.
    ...
    protected final int readahead() throws IOException {
        for (;;) {
            ...
            if (r5[1] != 3 || len <= 0) {
                ...
                // emulate the 4 bytes since we just try to read the handshake message header
     ...
     * @exception java.io.IOException
     *                throws an IOException if an I/O Error occurs, an alert is received or a type mismatch is detected.
     */
     ...
     * @throws IOException
     *             / final void hsWrite(int msgType, int length, int add) throws IOException { byte b[] = writeBuffer;
     *             b[5] = (byte)0x msgType; b[6] = (byte)0x (length >> 16); b[7] = (byte)0x (length >> 8); b[8] =
     *             (byte)0x (length);
     * 
     *             length += 4; if (DEBUG.ON) Misc.dump("write", System.out, b, 5, length); hsMd5.update(b, 5, length);
     *             hsSha.update(b, 5, length ); if (DEBUG.ON) Misc.dump("hsMd5", System.out,
     *             ((MD5)hsMd5.clone()).digest());
     * 
     *             b[0] = (byte)0x 22; b[1] = (byte)0x 3; // version b[2] = (byte)0x 0; b[3] = (byte)0x (length >>> 8);
     *             b[4] = (byte)0x (length); os.write(b, 0, length + 5 + add); }
     * 
     ...
     * @exception java.io.IOException
     *                throws an IOException if an I/O Error occurs. / final void hs_write(byte b[], int msgType) throws
     *                IOException { byte w0[] = new byte[b.length + 4]; w0[0] = (byte)0x msgType; w0[1] = (byte)0x
     *                (b.length >> 16); w0[2] = (byte)0x (b.length >> 8); w0[3] = (byte)0x (b.length);
     *                System.arraycopy(b, 0, w0, 4, b.length); rawwrite(w0, 22); }
     * 
     *                /** calculate a message MAC. hash(secret + pad_2 + hash (secret + pad_1 + seq_num + typ + length +
     *                content)); this function is synchronized since the same MessageDigest object is used for reading
     *                and writing.
     * @param md
     ...
     */
    final synchronized byte[] calcMessageHash(MessageDigest md, int hlen, byte secret[], long seqNum, int typ,
    ...
    /**
     * create some hash bytes md5(x + sha('A' + x + ra + readBuffer)) + md5(x + sha('BB' + x + ra + readBuffer)) + md5(x
     * + sha('CCC' + x + ra + readBuffer)) + ...
     * 
     * @param n
    ...
    /**
     * Assign InputStream and OutputStream for reading and writing to the underlying layer (often socket streams).
     * 
     * @param i
     ...
     * 
     * @return the used ciphertype 3 = SSL_RSA_WITH_RC4_40_MD5 4 = SSL_RSA_WITH_RC4_128_MD5 5 = SSL_RSA_WITH_RC4_128_SHA
     */
     ...
     * 
     * @param b
     *            a byte array wherein all zeros are replaced by non zero random values
     */
     ...
     *         0x84: 0x10, search first sequence, do not enter 0x90, search next sequence, ENTER that sequence 0x84,
     *         search bitstring, return its content!
     * @param b
    ...
    /**
     * Insert 4 bytes of a handshake message header at given offset. Rest of message follows 4 bytes behind the header
    ...
    /**
     * Insert 5 bytes of a packet message header at given offset. Rest of message follows 5 bytes behind the header and
     ...
     * 
     * byte secret[] = Misc.hex2Bytes( "6d eb 86 48 1f f7 68 a1  d5 d7 f0 c6 ee 92 1e 52"+
     * "21 10 05 5f cd 20 19 77  91 a9 24 f4 bb 35 8e cf" ); byte seed[] = Misc.hex2Bytes(
     * "6d 61 73 74 65 72 20 73  65 63 72 65 74 48 fb 56"+ "9e fb 8b da 61 d8 21 1c  a6 e5 94 26 b8 34 a5 97"+
     * "0f c8 1b c9 b3 89 fd d0  3f 81 ea d8 8b 48 fb 56"+ "9e e1 1a 1e 83 6b 09 7f  25 7d fc ed 59 10 a8 e2"+
     * "ef d1 e1 5e 30 66 33 c1  0b 67 c5 5b 13" );
     * 
     * Misc.dump("secret", System.out, secret); Misc.dump("seed", System.out, seed);
     * 
     * byte r[] = pHash(48, new MD5(), secret, seed); Misc.dump("result", System.out, r);
     * 
     * secret = Misc.hex2Bytes( "b2 8f a0 6c 81 c0 5d be  ee 90 4f 3b 51 c5 7e fc"+
     * "24 96 d6 8f c7 8c a3 e7  43 f4 d9 fc 81 33 b9 fa" ); r = pHash(48, new SHA(), secret, seed); Misc.dump("result",
     * System.out, r);
     * 
     * 
     * 
...
/**
 * $Log: Ssl3.java,v $
 * Revision 1.23  2014/09/22 09:16:24  bebbo
 ...
 * @I client adds current time to client random Revision 1.19 2013/05/17 10:54:38 bebbo
 * 
 * @D more DEBUG infos
 * @B errors yield a SslException which are 100ms tick aligned to avoid timing attacks Revision 1.18 2012/08/19 15:26:40
 *    bebbo
 * 
 * @N added SHA256
 ...
 * @V 1.0.2 Revision 1.14 2010/12/17 23:25:03 bebbo /FIXED: ssl config now supports multiple certificates Revision 1.13
 *    2009/02/05 20:06:28 bebbo
 * 
 ...
 * @I changed close_notify alert handling: no longer raises as an exception is now always sent on close()
 * 
