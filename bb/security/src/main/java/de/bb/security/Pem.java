...
public class Pem {
  ...
  {  
    ...
    if (procType == null || !procType.equalsIgnoreCase("4,ENCRYPTED")) {
    	int start = Mime.searchBegin(b, off);
    	// check for more PEM entries and add them all.
    	int next = Mime.searchBegin(b, start + 8);
    	if (next > 0) {
    		ByteRef all = new ByteRef(d);
	    	for (; next > 0; next = Mime.searchBegin(b, next + 8)) {
	    		// append further pem entries
	    	    byte [] add = Mime.searchDecode(b, next);
	    	    if (add == null)
	    	    	break;
	    	    all = all.append(add);
	    	}
	 ...
	 *            - input data
	 * @param name
	 *            - additional name for the encoded block: -----BEGIN <name>
	 * @param off
	 *            - offset into data
	 * @param pwd
	 *            - supplied password, needed for encryption
	 * @param salt
	 *            - a supplied salt, when null some salt is generated
	 * @return a new allocated byte array, when encoding was successful, either
	 *         null.
	 * @see decode
	 */
...
/**
 * $Log: Pem.java,v $ Revision 1.5 2002/11/06 09:46:12 bebbo
 * 
