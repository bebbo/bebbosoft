...
public class RemoteClient
{
  ...
  // holds all clients, if than 1 rmi connection is used.
  ...
  private LongHash refs = new LongHash();
...
//      os.writeUTF(pwd);
      ...
      synchronized (freeList) {
        if (freeList.size() > 0) {
          ...
          for (int i = freeList.size(); --i >= 0;) {
            ss.os.writeLong(((Long) freeList.get(i)).longValue());
          }
   ...
   */
  public Object invoke(String className, long oid, int fx)
      throws RemoteException
  {
    Streams s = null;
    try {
...
//      ex.printStackTrace();
   ...
   * @throws RemoteException
   */
  public Object invoke(String className, long oid, int fx, Object[] params)
      throws RemoteException
  {
  ...
  {
    refs.put(oid, new SoftReference(ref));
  }

  RemoteRef get(long oid)
  {
    SoftReference sr = (SoftReference) refs.get(oid);
    if (sr == null)
      return null;
    return (RemoteRef) sr.get();
  }
  ...
  {
    return (RemoteClient) clients.get(new Long(cid));
  }
    ...
    Runtime.getRuntime().addShutdownHook(new Thread()
    {
      public void run()
      {
        ...
        for (Iterator i = clients.values().iterator(); i.hasNext();) {
          RemoteClient c = (RemoteClient) i.next();
          try {
            c.checkGC();
          } catch (Exception e) {
          }
