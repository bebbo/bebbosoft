...
/**
 * Used to manage threads for the same task, when the count of needed tasks
 * changes with the time. A good example is a HTTP server where each request
 * gets an own task to process the request and computes the output. It's easy
 * understandable that the count of needed threads depends on the incoming
 * requests, the duration of the processing and so on. <br>
 * The complete management is done by this class, only the count of waiting
 * threads, ready to catch the next incoming request, and a maximum count of
 * active threads, to limit and protect from overload must be configured. <br>
 * An important hint is: Don?t use more waiting threads for one task than
 * available CPU.
 * <hr>
 * An example:
 * 
...
public class ThreadManager {
	...
	/**
	 * Interface which contains the thread creating function. The implementation
	 * has to create a thread and to start it.
	 */
	...
	public static class Thread extends java.lang.Thread {
		...
		/**
		 * This function MUST be called from the threads main loop BEFORE the
		 * real work starts.
		 */
		...
		/**
		 * Force termination of the current thread. Next call of mustDie() will
		 * return true;
		 */
		...
		/**
		 * Is the current thread busy? This functions gives only then correct
		 * information, if the functions setBusy() and mustDie() are used
		 * correctly.
		 * 
	...
	String mid;
	int tid = 0;
	// a thread group for all threads - used to detrmine the real count of
	// threads!
	...
	/**
	 * Replace each running thread with a new thread. Which means each running
	 * thread is terminated, and a new thread is started.
	 */
	public void renew() {
		synchronized (lock) {
	...
	private void check() {
		synchronized (lock) {
			...
			if (running + waiting + idling > threadGroup.activeCount()) {
				// System.out.println("fixup running thread count by " +
				// (threadGroup.activeCount() - waiting));
				running = threadGroup.activeCount() - waiting - idling;
			}
