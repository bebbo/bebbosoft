...
/**
 * A collection of high speed date conversion functions. If you ever measured
 * the really low performance of the JDK DateFormat classes, you understand its
 * purpose instantly. The most needed date/time formatting functions plus the
...
public class DateFormat {
	...
	final private static char[][] DAYNAMES = { { 'T', 'h', 'u' }, { 'F', 'r', 'i' }, { 'S', 'a', 't' },
			{ 'S', 'u', 'n' }, { 'M', 'o', 'n' }, { 'T', 'u', 'e' }, { 'W', 'e', 'd' } };

	final private static char[][] MONTHNAMES = { { 'J', 'a', 'n' }, { 'F', 'e', 'b' }, { 'M', 'a', 'r' },
			{ 'A', 'p', 'r' }, { 'M', 'a', 'y' }, { 'J', 'u', 'n' }, { 'J', 'u', 'l' }, { 'A', 'u', 'g' },
			{ 'S', 'e', 'p' }, { 'O', 'c', 't' }, { 'N', 'o', 'v' }, { 'D', 'e', 'c' } };

	final private static byte[][] BDAYNAMES = { { 'T', 'h', 'u' }, { 'F', 'r', 'i' }, { 'S', 'a', 't' },
			{ 'S', 'u', 'n' }, { 'M', 'o', 'n' }, { 'T', 'u', 'e' }, { 'W', 'e', 'd' } };

	final private static byte[][] BMONTHNAMES = { { 'J', 'a', 'n' }, { 'F', 'e', 'b' }, { 'M', 'a', 'r' },
			{ 'A', 'p', 'r' }, { 'M', 'a', 'y' }, { 'J', 'u', 'n' }, { 'J', 'u', 'l' }, { 'A', 'u', 'g' },
			{ 'S', 'e', 'p' }, { 'O', 'c', 't' }, { 'N', 'o', 'v' }, { 'D', 'e', 'c' } };
	...
	/**
	 * This constructor deals with various date formats. Supported abbreviations
	 * are:
	 * <ul>
	 * <li>d - day</li>
	 * <li>M - month</li>
	 ...
	 * </ul>
	 * 
	 * if 2 or less characters are used, the short form is used (if any exist).
	 * Otherwise leading zeros are inserted to reach the defined amount.
	 * 
	 * Month: - M yields the numeric months suppressing the leading zero - MM yields
	 * the numeric months with a leading zero - MMM yields the English short name -
	 * MMMM or more ist not yet implemented
	 * 
	 * Milliseconds: - S yields 1/10 seconds - SS yields 1/100 seconds - SSS yields
	 * 1/1000 seconds
	 * 
	 ...
	 */
	public DateFormat(final String format, final int tz) {
		myTimeZone = tz;

		LinkedList<int[]> fieldInfos = new LinkedList<int[]>();
...
//				}
		...
		int n = 0;
		for (Iterator<int[]> i = fieldInfos.iterator(); i.hasNext();) {
			int[] ii = (int[]) i.next();
			indexes[n] = ii[0];
			offsets[n] = ii[1];
			lengths[n] = ii[2];
	...
	/**
	 * Returns the current fill bytes. Means all date/time digits are replaces by
	 * spaces.
	 * 
	 * @return Returns the current fill bytes. Means all date/time digits are
	 *         replaces by spaces.
	 */
	...
	/**
	 * Returns the formatted String using the format String specified at the
	 * constructor. Important to know that this is much faster than the
	 * java.util.DateFormat, it is still 3 times slower than the explicit (hard
	 * coded) static format functions.
	 * 
	 * @param time GMT time in milliseconds.
	 * @return Returns the formatted String using the format String specified at the
	 *         constructor.
	 */
	 ...
	 * @param val
	 * @param offset     into the byte array b
	 * @param len        length to fill
	 * @param defaultLen default length
	 */
	private void lfill(final byte[] b, int val, int offset, int len, int defaultLen) {
		// fill leading zeros
	 ...
	 * @param len
	 */
	private void fill(final byte[] b, int val, final int offset, final int len) {
	...
	/**
	 * Return a byte array for the given date (long) in format
	 * <code>dd/MMM/yyyy:HH:mm:ss +zz:zz</code>.
	 * 
	 * <pre>
	 ...
	 * .
	 * 
	 * @param l a long representing a date value
	 * @return a byte array for the given date (long) in format
	 *         </code>dd/MMM/yyyy:HH:mm:ss +zz:zz</code>.
	 */
	public static byte[] ba_dd_MMM_yyyy_HH_mm_ss_zzzz(final long l) {
		int[] v = calc(l);
		byte b[] = new byte[26];
	...
	/**
	 * Return a String for the given date (long) in format
	 * <code>dd/MMM/yyyy:HH:mm:ss +zz:zz</code>.
	 * 
	 * <pre>
	 * e.g. 01/Jan/1970:00:00:01 +02:00
	 ...
	 *         </code>dd/MMM/yyyy:HH:mm:ss +zz:zz</code>.
	 */
	public static String dd_MMM_yyyy_HH_mm_ss_zzzz(final long l) {
		int[] v = calc(l);
		char b[] = new char[26];
	...
	/**
	 * Return a String for the given date (long) in format
	 * <code>dd/MMM/yyyy:HH:mm:ss GMT+zz:zz</code>.
	 * 
	 * <pre>
	 ...
	 *         <code>dd/MMM/yyyy:HH:mm:ss GMT+zz:zz</code>.
	 */
	public static String dd_MMM_yyyy_HH_mm_ss_GMT_zz_zz(final long l) {
		int[] v = calc(l);
		char b[] = new char[30];
	...
	/**
	 * Return a String for the given date (long) in format
	 * <code>EEE, dd MMM yyyy HH:mm:ss +zzzz</code>.
	 * 
	 * <pre>
	 ...
	 *         <code>EEE, dd MMM yyyy HH:mm:ss +zzzz</code>.
	 */
	public static String EEE__dd_MMM_yyyy_HH_mm_ss__zzzz(final long l) {
		int[] v = calc(l);
		char b[] = new char[31];
	...
	/**
	 * Return a byte array for the given date (long) in format
	 * <code>EEE, dd MMM yyyy HH:mm:ss GMT</code>.
	 * 
	 * <pre>
	 ...
	 * 
	 * @param l a long representing a date value
	 * @return a byteA array for the given date (long) in format
	 *         <code>EEE, dd MMM yyyy HH:mm:ss GMT</code>.
	 */
	public static byte[] ba_EEE__dd_MMM_yyyy_HH_mm_ss_GMT(final long l) {
		int[] v = calc(l, 0);
		byte b[] = new byte[29];
	 ...
	 *         <code>EEE, dd MMM yyyy HH:mm:ss GMT</code>.
	 */
	public static String EEE__dd_MMM_yyyy_HH_mm_ss_GMT(final long l) {
		int[] v = calc(l, 0);
		char b[] = new char[29];
	 ...
	 * @return a String for the given date (long) in format yyyyMMddHHmmss.
	 */
	public static String yyyyMMddHHmmss(final long l) {
		int[] v = calc(l);
		char b[] = new char[14];
		int y = v[0]; // year
		b[3] = (char) (y % 10 + 48);
	 ...
	 * @return a String for the given date (long) in format yyyyMMddHHmmssSSS.
	 */
	public static String yyyyMMddHHmmssSSS(final long l) {
		int[] v = calc(l);
		char b[] = new char[17];
		int y = v[0]; // year
		b[3] = (char) (y % 10 + 48);
	 ...
	 * @return a String for the given date (long) in format ddMMyyyyHHmmss.
	 */
	public static String ddMMyyyyHHmmss(final long l) {
		int[] v = calc(l);
		char b[] = new char[14];
		int y;
	 ...
	 * @return a String for the given date (long) in format yyyyMMddHH.
	 */
	public static String yyyyMMddHH(final long l) {
		int[] v = calc(l);
		char b[] = new char[10];
		int y = v[0];
		b[3] = (char) (y % 10 + 48);
	 ...
	 * @return a String for the given date (long) in format yyyyMMdd.
	 */
	public static String yyyyMMdd(final long l) {
		int[] v = calc(l);
		char b[] = new char[8];
		int y = v[0];
		b[3] = (char) (y % 10 + 48);
	 ...
	 * @return a String for the given date (long) in format ddMMyyyy.
	 */
	public static String ddMMyyyy(final long l) {
		int[] v = calc(l);
		char b[] = new char[8];
		int y;
		y = v[2] + 1; // day
	 ...
	 */
	public static int[] calc(final long l) {
	 ...
	 * 
	 * @param l        a long representing a date value
	 * @param timeZone an explicit time zone
	 * @return a new allocated array containing the calculated values
	 */
	public static int[] calc(long l, final int timeZone) {
	...
	private static long calcLong(int year, final int mm, final int day, final int hour, final int minutes,
			final int seconds, final int tzhour, final int tzmin, final int sig) {
		--year;
	 ...
	 * 
	 * @param n the int array as calculated by calc.
	...
	/**
	 * Return a long for the given date date in format
	 * <code>dd MMM yyyy HH mm ss +zz zz</code> also values without timezone or
	 * without HH mm ss are parsed!
	 * 
	 * <pre>
	 * e.g. 01/Jan/1970:00:00:01 GMT+02:00
	 ...
	 *         <code>dd MMM yyyy HH mm ss +zz zz</code>
	 */
	public static long parse_dd_MMM_yyyy_HH_mm_ss_GMT_zz_zz(final String date) {
	 ...
	 * @return
	 */
	private static int nextDigitOrSign(final byte[] b, int i) {
	 ...
	 * @return
	 */
	private final static int skipSpaces(final byte[] b, int i) {
	...
	/**
	 * Return a long for the given date date in format
	 * <code>dd MM yyyy HH mm ss +zz zz</code> also values without timezone or
	 * without HH mm ss are parsed!
	 * 
	 * <pre>
	 ...
	 *         <code>dd MMM yyyy HH mm ss +zz zz</code>
	 */
	public static long parse_dd_MM_yyyy_HH_mm_ss_GMT_zz_zz(final String date) {
		byte[] b = date.getBytes();
		try {
			int val[] = new int[6];
	...
	/**
	 * Return a long for the given date date in format
	 * <code>yyyy MM dd HH mm ss +zz zz</code> also values without timezone or
	 * without HH mm ss are parsed!
	 * 
	 * <pre>
	 ...
	 *         <code>dd MMM yyyy HH mm ss +zz zz</code>
	 */
	public static long parse_yyyy_MM_dd_HH_mm_ss_GMT_zz_zz(final String date) {
		byte[] b = date.getBytes();
		try {
			int val[] = new int[6];
	 ...
	 * @return
	 */
	private static boolean isGMT(final byte[] b, final int i) {
		if (b.length < i + 3)
			return false;
	 ...
	 * @return
	 */
	private final static int nextSeconds(final int[] val, final byte[] b, int i) {
		int seconds = b[i++] - '0';
		if (b[i] >= '0' && b[i] <= '9') {
			seconds *= 10;
			seconds += b[i++] - '0';
	 ...
	 * @return
	 */
	private final static int nextMinutes(final int[] val, final byte[] b, int i) {
		int minutes = b[i++] - '0';
		if (b[i] >= '0' && b[i] <= '9') {
			minutes *= 10;
			minutes += b[i++] - '0';
	 ...
	 * @return
	 */
	private final static int nextHour(final int[] val, final byte[] b, int i) {
		int hour = b[i++] - '0';
		if (b[i] >= '0' && b[i] <= '9') {
			hour *= 10;
			hour += b[i++] - '0';
	 ...
	 * @return
	 */
	private final static int nextDay(final int[] val, final byte[] b, int i) {
		int day = b[i++] - '0';
		if (b[i] >= '0' && b[i] <= '9') {
			day *= 10;
			day += b[i++] - '0';
	 ...
	 * @return
	 */
	private final static int nextMonth(final int[] val, final byte[] b, int i) {
		int mm = b[i++] - '0';
		if (b[i] >= '0' && b[i] <= '9') {
			mm *= 10;
			mm += b[i++] - '0';
	 ...
	 * @return
	 */
	private final static int nextYear(final int[] val, final byte[] b, int i) {
		int year = b[i++] - '0';
		if (b[i] >= '0' && b[i] <= '9') {
			year *= 10;
			year += b[i++] - '0';
	 ...
	 * @return
	 */
	private final static int nextDigit(final byte[] b, int i) {
		// search next digit
	 ...
	 * @return a time value for the easter day within that year in milli seconds
	 *         since 01.01.1970
	 */
	public static long eastern(final int year) {
		// always 21.03.yyyy
		...
		return toLong(v);
		/*
		 * // add days and subtract hhmmss long easterDay = nextFullMoon + ((((days * 24
		 * - v[4]) * 60 - v[5]) * 60 - v[6] + offsetSeconds) 1000L); //
		 * System.out.println("Ostern ist " +
	...
	/*
	 * public static void main(String argso[]) { eastern(1999); eastern(2000);
	 * eastern(2001); eastern(2002);
	 * 
	 * long t = parse_yyyy_MM_dd_HH_mm_ss_GMT_zz_zz("20000228");
	 * System.out.println(EEE__dd_MMM_yyyy_HH_mm_ss__zzzz(t));
	 ...
	 * parse_yyyy_MM_dd_HH_mm_ss_GMT_zz_zz("20001231");
	 * System.out.println(EEE__dd_MMM_yyyy_HH_mm_ss__zzzz(t));
	 * System.out.println(EEE__dd_MMM_yyyy_HH_mm_ss__zzzz(t + timePerDay));
	 * System.out.println(EEE__dd_MMM_yyyy_HH_mm_ss__zzzz(t + timePerDay * 2)); t =
	 * parse_yyyy_MM_dd_HH_mm_ss_GMT_zz_zz("20010228");
	 * System.out.println(EEE__dd_MMM_yyyy_HH_mm_ss__zzzz(t));
	 * System.out.println(EEE__dd_MMM_yyyy_HH_mm_ss__zzzz(t + timePerDay));
	...
	public static void initTimeZone() {
		...
		/*
		 * long l = System.currentTimeMillis(); java.text.DateFormat df = new
		 * SimpleDateFormat("yyyyMMddHH"); long t0 = Long.parseLong(df.format(new
		 * Date(l))); long t1 = Long.parseLong(yyyyMMddHH(l)); offsetMillis = 3600 *
		 * (int) (t0 - t1);
		 */
		if (DEBUG) {
	...
	/**
	 * / public static void main(String args[]) { long l1 =
	 * parse_dd_MMM_yyyy_HH_mm_ss_GMT_zz_zz("01.Jan.1970"); long l2 =
	 * parse_dd_MM_yyyy_HH_mm_ss_GMT_zz_zz("02.01.2003"); long l3 =
	 * parse_dd_MM_yyyy_HH_mm_ss_GMT_zz_zz("03.01.2003"); long l4 =
	 * parse_dd_MM_yyyy_HH_mm_ss_GMT_zz_zz("04.01.2003"); long l5 =
	 * parse_dd_MM_yyyy_HH_mm_ss_GMT_zz_zz("05.01.2003"); long l6 =
	 * parse_dd_MM_yyyy_HH_mm_ss_GMT_zz_zz("06.01.2003");
	 * 
	 * System.out.println(dd_MMM_yyyy_HH_mm_ss_GMT_zz_zz(l1));
	 * System.out.println(dd_MMM_yyyy_HH_mm_ss_GMT_zz_zz(l2));
	 * System.out.println(dd_MMM_yyyy_HH_mm_ss_GMT_zz_zz(l3));
	 * System.out.println(dd_MMM_yyyy_HH_mm_ss_GMT_zz_zz(l4));
	 * System.out.println(dd_MMM_yyyy_HH_mm_ss_GMT_zz_zz(l5));
	 * System.out.println(dd_MMM_yyyy_HH_mm_ss_GMT_zz_zz(l6));
	 * 
	 * 
	 * 
	 * DateFormat df = new DateFormat("dd.MM.yy HH:mm:ss+SSS", 0);
	 * System.out.println(df.format(System.currentTimeMillis())); df = new
	 * DateFormat("d.M.y H:m:s+S TZ=z", 0);
	 * System.out.println(df.format(System.currentTimeMillis())); df = new
	 * DateFormat("d.MMMMMM.yyyy H:m:s+S TZ=z", 0);
	 * System.out.println(df.format(System.currentTimeMillis())); df = new
	 * DateFormat("yyyyMMddHHmmssSSS", 0);
	 ...
	 * 
	 * long diff = System.currentTimeMillis() - now; System.out.println("df: " +
	 * diff);
	 * 
	 ...
	 * 
	 * @param date a GMT time in milli seconds.
	 * @return the value for date plus 1 week.
	 */
	public static long nextWeek(final long date) {
		return date + 7 * TIME_PER_DAY;
	}

	/**
	 * Returns the value for date plus 1 month. If same day does not exist the last
	 * possible day is used.
	 * 
	 * @param date a GMT time in milli seconds
	 * @return the value for date plus 1 month.
	 */
	public static long nextMonth(final long date) {
	 ...
	 */
	public static long addMonths(final long t, final int months) {
		int n[] = calc(t);
		n[1] += months;
		n[0] += n[1] / 12;
		n[1] = n[1] % 12;
	 ...
	 * 
	 * @param date a time in milli seconds
	 * @return the value for the first day in date's month.
	 */
	public static long firstOfMonth(final long date) {
		int n[] = calc(date);
	 ...
	 */
	public static long firstOfYear(final long date) {
		int n[] = calc(date);
	 ...
	 * 
	 * @param time a time in milli seconds with current timezone
	 * @return int the week number of the day
	 */
	public static int getWeek(final long time) {
		int n[] = calc(time);
		n[1] = 0;
		n[2] = 0;
		long jan1 = toLong(n);
	 ...
	 */
	public static int getDayOfMonth(final long date) {
		int n[] = calc(date);
		return n[2] + 1;
	}
	 ...
	 */
	public static int getMonth(final long t) {
		int[] v = calc(t);
		return v[1] + 1;
	}
	 ...
	 */
	public static int getYear(final long t) {
		int[] v = calc(t);
		return v[0];
	}

	/**
	 * Returns a time value for the specified day at 00:00:00.000 with current time
	 * zone.
	 * 
	 * @param t a time value
	 * @return a time value for the specified day at 00:00:00.000 with current time
	 *         zone.
	 */
	public static long firstOfDay(final long t) {
		int n[] = calc(t);
		n[4] = n[5] = n[6] = n[7] = 0;
		return toLong(n);
	}
...
//		long t = parse_dd_MM_yyyy_HH_mm_ss_GMT_zz_zz("010210");
		long t = new DateFormat("ddMMyy").parse("010210");
		String d = new DateFormat("dd.MM.yyyy").format(t);
